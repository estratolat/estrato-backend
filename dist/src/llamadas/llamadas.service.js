...
var LlamadasService_1;
Object.defineProperty(exports, "__esModule", { value: true });
exports.LlamadasService = void 0;
const common_1 = require("@nestjs/common");
const prisma_service_1 = require("../common/services/prisma.service");
const server_sdk_1 = require("@vapi-ai/server-sdk");
let LlamadasService = LlamadasService_1 = class LlamadasService {
    constructor(prisma) {
        this.prisma = prisma;
        this.logger = new common_1.Logger(LlamadasService_1.name);
    }
    getProveedorClient() {
        ...
        return new server_sdk_1.VapiClient({ token });
    }
    ...
    async importarVotantes(id, tenantId, votanteIds) {
        const campana = await this.findOne(id, tenantId);
        if (!votanteIds.length)
            throw new common_1.BadRequestException('No se enviaron votantes');
        ...
        if (!votantes.length)
            throw new common_1.BadRequestException('No se encontraron votantes con teléfono');
        const creadas = [];
    ...
    async iniciarLlamada(campanaId, votanteId, tenantId) {
        ...
        if (!llamada)
            throw new common_1.NotFoundException('Llamada no encontrada en la campaña');
        ...
        if (!assistantId || !phoneNumberId) {
            throw new common_1.BadRequestException('Falta configurar assistant_id y phone_number_id en la campaña o variables de entorno');
        }
        const client = this.getProveedorClient();
        let proveedorCallId = null;
        if (client) {
            try {
                ...
                throw new common_1.BadRequestException('No se pudo iniciar la llamada automática');
            }
        }
        else {
    ...
    async procesarWebhook(payload) {
        ...
        const telefono = this.limpiarTelefono(msg.customer?.number || msg.phoneNumber?.number || '');
    ...
    async getLlamadas(campanaId, tenantId) {
    ...
    normalizarStatus(status) {
        ...
        if (['completed', 'done', 'ended', 'success'].includes(s))
            return 'completada';
        if (['in-progress', 'in_progress', 'ongoing'].includes(s))
            return 'en_curso';
        if (['no-answer', 'no_answer', 'unanswered'].includes(s))
            return 'no_contesta';
        if (['busy', 'failed', 'error'].includes(s))
            return 'fallida';
        if (['voicemail', 'machine'].includes(s))
            return 'buzon';
        if (s === 'contestada')
            return 'contestada';
        return s;
...
exports.LlamadasService = LlamadasService;
exports.LlamadasService = LlamadasService = LlamadasService_1 = __decorate([
    (0, common_1.Injectable)(),
    __metadata("design:paramtypes", [prisma_service_1.PrismaService])
], LlamadasService);
//# sourceMappingURL=llamadas.service.js.map